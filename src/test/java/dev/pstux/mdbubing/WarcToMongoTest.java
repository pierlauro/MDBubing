...
public class WarcToMongoTest {
  ...
  public void validRecordsAreInsertedInCollection() throws Exception {
    ...
    WarcToMongo.main(
        new String[] {
          "-P", WarcToMongoTest.class.getResource(TEST_WARC_TO_MONGO_PROPERTIES_PATH).getPath()
        });

    // Verify that valid records are inserted in the collection
    verify(mockCollection, times(NUM_VALID_TEST_WARC_RECORDS))
        .insertOne(Mockito.any(Document.class));
  }
}
